...
class dqModel(object):
    ...
    def _initializeVariables(self, temperature):
        ...
        self.R = self.nameplate['Resistance (Ohm)']  # + self.machine.controlcircuit.Rsource / 3
    ...
    def __calculateMotorData(self, speed=None, torque=None):
        ...
        if speed:
            ...
            while (i < Imax):
                ...
                PlossesMotor = self.__getAllMotorLosses(speed, Irms)
                PlossesECU = self.machine.controlcircuit.getLosses(Isource, Irms, Iripple)["Total Losses (W)"]
                # Somehow this ratio helps so that the DC current conveges to right value at high source resistance losses
                ratio = 1  # self.speed_max(0, self.InnerTorque(0, torque)) / speed
                etaECU = 100 * Pm / (Pm + PlossesECU * ratio)
            ...
            if maxSpeed and speed > maxSpeed:
                # Wanted speed above the speed-torque characteristics
                return None

            # U0 = self.machine.controlcircuit.Vdc - self.machine.controlcircuit.Rsource * Isource
            # ratio = speed / self.speed_max(0, self.InnerTorque(0, torque))
            # Isource = Isource * U0 / Umax / math.sqrt(3) if ratio > 0.9 else Isource
            # print("speed", speed, "maxSpeed", self.speed_max(0, self.InnerTorque(0, torque)), "U0", U0, "Umax", Umax * math.sqrt(3), "ratio", ratio)

        else:
            # Calculates the point in the performance curve.
            Isource0 = 1
            while (i < Imax):
                ...
                PlossesECU = self.machine.controlcircuit.getLosses(Isource, Irms, Iripple)["Total Losses (W)"]
                PlossesMotor = self.__getAllMotorLosses(speed, Irms)
                etaECU = 100 * Pm / (Pm + PlossesECU)
                delta = 100 * abs(Isource - Isource0) / Isource0
                U0 = self.machine.controlcircuit.Vdc - self.machine.controlcircuit.Rsource * Isource
        ...
        PlossesECU = electronic["Total Losses (W)"]
        etaMotor, etaECU = 100 * Pout / Pm,  100 * Pm / (Pm + PlossesECU)
        etaTotal = etaECU * etaMotor / 100
        etaContacts = 100 * Pm / (Pm + electronic["Contacts Losses (W)"])
        etaPowerStage = 100 * (Pm + electronic["Contacts Losses (W)"]) / (Pm + electronic["Contacts Losses (W)"] + electronic["Power Stage Losses (W)"])
        etaSource = 100 * (Pm + electronic["Contacts Losses (W)"] + electronic["Power Stage Losses (W)"]) / (Pm + PlossesECU)

...
class blckModel(object):
    ...
    def __initializeVariables(self, temperature):
        ...
        useECUTemperature = self.data["variation"]["design"].get(
            "changeECUTemperature", False)
        if not useECUTemperature:
            self.data["variation"]["design"]["Control Circuit"][
                "Used"]["Temperature (C)"] = self.temperature

        validation = validate(pmMachine(self.data["variation"], recomputeGeometry=False), pmMachine(
        ...
        self.pp = int(self.data["variation"]["design"]
                      ["Rotor"]["Pole Number"]) / 2
        self.useReluctance = True
        self.frictionTorque = self.data["variation"][
            "design"]["Mechanics"]["Friction Torque (Nm)"]
        self.momentOfInertia = self.data["variation"][
            "design"]["Mechanics"]["Moment of Inertia (kg*m^2)"]
        self.damping = self.data["variation"][
    ...
    def calculatePerformance(self):
        ...
        if not len(self.loads):
            self.__initializeVariables(
                self.data["variation"]["design"]["Environment"]["Ambient Temperature (C)"])
            results.append({
            ...
            for load in self.loads:
                ...
                performance = self.__getPerformance()
                loadPoints = self.__getLoadPoints(
                    performance, load["loadPoints"])
    ...
    def __getLoadPoints(self, performance, loadPoints):
        ...
        for loadPoint in loadPoints:
            ...
            if index == None:
                data.append(
                    {"Speed (rpm)": loadPoint["speed"], "Torque (Nm)": loadPoint["torque"], "Possible": False})
            else:
                result = self.__getInterpolatedData(
                    loadPoint["speed"], loadPoint["torque"], performance, index)
        ...
        for item in loadPoints:
            if item['Torque (Nm)'] < torqueLimit:
                ...
                filteredLoadPoints.append(
                    {"Speed (rpm)": item["Speed (rpm)"], "Torque (Nm)": item["Torque (Nm)"], "Voltage Limit": True})
    ...
    def __calculateMotorData(self, loadSpeed=None, loadTorque=None):
        ...
        while (i < Nmax and delta > epsilon):
            self.Vdc = self.controlcircuit["Power Source"]['Supply Voltage (V)'] * \
                etaECU / 100
            ...
            (Isource, Imax, Iavg, Irms, Iripple, Tshaft) = self.solveSpeed(
                loadSpeed=speed, numberOfPeriods=2, totalTimeSteps=self.totalTimeSteps)
            if Tshaft <= 0:
                etaECU = 100
            ...
            etaECU = 100 * (Pout + PFakelossesMotor) / \
                (Pout + PFakelossesMotor + PAdditionallossesECU)
        ...
        Isource = (Pout + PlossesMotor + PlossesECU) / \
            self.machine.controlcircuit.Vdc
        electronic = self.machine.controlcircuit.getLosses(
            Isource, Irms, Iripple)
        etaMotor = 100 * Pout / (Pout + PlossesMotor)
        etaContacts = 100 * (Pout + PlossesMotor) / (Pout +
                                                     PlossesMotor + electronic["Contacts Losses (W)"])
        etaPowerStage = 100 * (Pout + PlossesMotor + electronic["Contacts Losses (W)"]) / (
            Pout + PlossesMotor + electronic["Contacts Losses (W)"] + electronic["Power Stage Losses (W)"])
        etaSource = 100 * (Pout + PlossesMotor + electronic["Contacts Losses (W)"] + electronic["Power Stage Losses (W)"]) / (
            Pout + PlossesMotor + electronic["Total Losses (W)"])
    ...
    def __getIndexOfLargerLoadPoint(self, loadPoint, performance):
        ...
        # returns 0 if conion is not fulfilled.
        index = np.argmax(t >= __innerTorque)

        if index == 0:
            return None
        else:
            __speed = np.interp(__innerTorque, [performance[index - 1]['Inner Torque (Nm)'], performance[index - 1]
                                ['Inner Torque (Nm)']], [performance[index]['Speed (rpm)'], performance[index]['Speed (rpm)']])
    ...
    def __getInterpolatedData(self, speed, torque, performance, index):
        ...
        __innerTorque = self.__getInnerTorqueFromShaftTorque(speed, torque)
        __speed = np.interp(__innerTorque, [p0['Inner Torque (Nm)'], p1['Inner Torque (Nm)']], [
                            p0['Speed (rpm)'], p1['Speed (rpm)']])
        Imax = np.interp(__innerTorque, [p0['Inner Torque (Nm)'], p1['Inner Torque (Nm)']], [
                         p0['Line Current MAX (A)'], p1['Line Current MAX (A)']])
        Iavg = np.interp(__innerTorque, [p0['Inner Torque (Nm)'], p1['Inner Torque (Nm)']], [
                         p0['Line Current AVG (A)'], p1['Line Current AVG (A)']])
        Irms = np.interp(__innerTorque, [p0['Inner Torque (Nm)'], p1['Inner Torque (Nm)']], [
                         p0['Line Current RMS (A)'], p1['Line Current RMS (A)']])
        Iripple = np.interp(__innerTorque, [p0['Inner Torque (Nm)'], p1['Inner Torque (Nm)']], [
                            p0['Capacitor Ripple Current (A)'], p1['Capacitor Ripple Current (A)']])
        __sourceCurrent = np.interp(__innerTorque, [p0['Inner Torque (Nm)'], p1['Inner Torque (Nm)']], [
                                    p0['Source Current (A)'], p1['Source Current (A)']])
        ...
        while (delta > 0.01):
            PAdditionallossesECU = self.machine.controlcircuit.getLosses(
                Isource0, Irms, Iripple)["Additional Losses (W)"]
            PFakelossesMotor = self.__getFakeMotorLosses(speed, Irms)
            Isource = (Pout + PFakelossesMotor + PAdditionallossesECU *
                       ratio) / self.machine.controlcircuit.Vdc
        ...
        Isource = Pout / etaTotal * 100 / \
            self.controlcircuit["Power Source"]['Supply Voltage (V)']
    ...
    def solveTorque(self,  electronic, loadTorque, numberOfPeriods, totalTimeSteps):
        ...
        Nskip = 10  # No need to reaf emf so fine! Only derivatives!
    ...
    def solveSpeed(self, loadSpeed, numberOfPeriods, totalTimeSteps):
    ...
    def L(self, speed, time, initangle, phaseadvance):
        w = self.pp * 2 * math.pi * speed / 60.0 * time
        delta = (initangle - phaseadvance) * DEG2RAD
        return self.Lg0 + self.Lg2 * math.cos(2 * w + delta)

    def dL(self, speed, time, initangle, phaseadvance):
        w = self.pp * 2 * math.pi * speed / 60.0 * time
        delta = (initangle - phaseadvance) * DEG2RAD
        return -(2 * self.pp * 2 * math.pi * speed / 60.0) * self.Lg2 * math.sin(w + delta)

    def M(self, speed, time, initangle, phaseadvance):
        w = self.pp * 2 * math.pi * speed / 60.0 * time
        delta = (initangle - phaseadvance) * DEG2RAD
        return -self.Lg0 / 2 + self.Lg2 * math.cos(2 * w + delta)

    def dM(self, speed, time, initangle, phaseadvance):
        w = self.pp * 2 * math.pi * speed / 60.0 * time
        delta = (initangle - phaseadvance) * DEG2RAD
        return -(2 * self.pp * 2 * math.pi * speed / 60.0) * self.Lg2 * math.sin(w + delta)

    def ke(self, position, initangle, phaseadvance):
        # 0 - Real, 1 - Imaginary
    ...
    def Un(self, time):
        ...
        if (self.Q1 == 1 and self.Q2 == 0 and self.Q3 == 0 and self.Q4 == 0 and self.Q5 == -1 and self.Q6 == 0 and self.D6 == 1):
            vm = ((2 - self.__PWM(time)) * self.Vdc - self.Ea - self.Eb - self.Ec
                  - self.Ia_phase * (self.dLaa + self.dLab + self.dLca) - self.Ib_phase * (
                      self.dLab + self.dLbb + self.dLbc) - self.Ic_phase * (self.dLca + self.dLbc + self.dLcc)
                  - (-self.dIb_phase - self.dIc_phase) * (self.Laa + self.Lab + self.Lca) - self.dIb_phase *
                  (self.Lab + self.Lbb + self.Lbc) -
                  self.dIc_phase * (self.Lca + self.Lbc + self.Lcc)
                  ) / 3.0
            va = self.Vdc
            ...
            vm = ((2 - self.__PWM(time)) * self.Vdc - self.Ea - self.Eb
                  - self.Ia_phase * (self.dLaa + self.dLab) - self.Ib_phase *
                  (self.dLbb + self.dLab) - 0 * (self.dLca + self.dLbc)
                  - (-self.dIb_phase - self.dIc_phase) * (self.Laa + self.Lab) -
                  self.dIb_phase * (self.Lbb + self.Lab) -
                  0 * (self.Lca + self.Lbc)
                  ) / 2.0
            va = self.Vdc
            vb = (1 - self.__PWM(time)) * self.Vdc
        ...
        # region AC (Q1 = 1, Q6 = PWM)
            ...
            vm = ((2 - self.__PWM(time)) * self.Vdc - self.Ea - self.Ec
                  - self.Ia_phase * (self.dLaa + self.dLca) - 0 * (self.dLab +
                                                                   self.dLbc) - self.Ic_phase * (self.dLca + self.dLcc)
                  - (-self.dIb_phase - self.dIc_phase) * (self.Laa + self.Lca) -
                  0 * (self.Lab + self.Lbc) -
                  self.dIc_phase * (self.Lcc + self.Lca)
                  ) / 2.0
            va = self.Vdc
            vb = self.Eb + vm  # ng because Ib = 0
            vc = (1 - self.__PWM(time)) * self.Vdc
            return (va, vb, vc, vm)
        ...
        # region BC (Q2 = 1, Q6 = PWM)
            ...
            vm = ((2 - self.__PWM(time)) * self.Vdc - self.Eb - self.Ec
                  - 0 * (self.dLab + self.dLca) - self.Ib_phase * (self.dLbb +
                                                                   self.dLbc) - self.Ic_phase * (self.dLbc + self.dLcc)
                  - (self.dIa_phase - self.dIc_phase) * (self.Lbb +
                                                         self.Lbc) - self.dIc_phase * (self.Lcc + self.Lbc)
                  ) / 2.0
            va = self.Ea + vm  # ng
            vb = self.Vdc
            vc = (1 - self.__PWM(time)) * self.Vdc
        ...
        # region BA (Q2 = 1, Q4 = PWM)
            ...
            vm = ((2 - self.__PWM(time)) * self.Vdc - self.Ea - self.Eb
                  - self.Ia_phase * (self.dLaa + self.dLab) - self.Ib_phase *
                  (self.dLab + self.dLbb) - 0 * (self.dLca + self.dLbc)
                  - (-self.dIb_phase - self.dIc_phase) * (self.Laa +
                                                          self.Lab) - self.dIb_phase * (self.Lbb + self.Lab)
                  ) / 2.0
            va = (1 - self.__PWM(time)) * self.Vdc
            vb = self.Vdc
            vc = self.Ec + vm  # ng
        ...
        # region CA (Q3 = 1, Q4 = PWM)
            ...
            vm = ((2 - self.__PWM(time)) * self.Vdc - self.Ec - self.Ea
                  - self.Ia_phase * (self.dLaa + self.dLca) - 0 * (self.dLab +
                                                                   self.dLbc) - self.Ic_phase * (self.dLca + self.dLcc)
                  - self.dIc_phase *
                  (self.Lcc + self.Lca) - (-self.dIc_phase -
                                           self.dIb_phase) * (self.Laa + self.Lca)
                  ) / 2.0
            va = (1 - self.__PWM(time)) * self.Vdc
            vb = self.Eb + vm
            vc = self.Vdc
    ...
    def __getSpeed(self, speed, loadTorque, shaftTorque):
        omega = ((shaftTorque - loadTorque) * self.timeStep) / \
            self.momentOfInertia + 2.0 * math.pi * speed / 60.0
        return omega / 2.0 / math.pi * 60

    def __getShaftTorque(self, speed, innerTorque):
    ...
    def __getElectricalAngle(self, speed, time):
        return self.pp * 2 * math.pi * speed / 60.0 * time * RAD2DEG

    def __getMechanicalAngle(self, speed, time):
        return 2 * math.pi * speed / 60.0 * time * RAD2DEG

    def __getInnerTorque(self, speed, time):
    ...
    def __getSimulationTime(self, speed, numberOfPeriods):

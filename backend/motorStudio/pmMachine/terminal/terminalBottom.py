...
class terminalBottom:
    """ Bottom terminal object. """

    def __init__(self, stator, winding):
        """ Constructor for the bottom terminal object. """
        self.stator = stator
        self.winding = winding

    def getCoordinates(self):
        """ Calculates the coordinates for the bottom terminal """
        ...
        l2 = line.__pointANDpoint__(p[1], p[2])
        l3 = line.__pointANDpoint__(p[2], p[3])
        l4 = line.__pointANDpoint__(p[len(p) - 3], p[len(p) - 4])
        l5 = line.__pointANDpoint__(p[len(p) - 2], p[len(p) - 3])
        l6 = line.__slopeANDpoint__(0, p[1])
        ...
        l6.moveParallel(self.winding.slotIsolation)

        p0 = l6.lineIntersection(l2)
        p1 = l2.lineIntersection(l3)
        p2 = l3.lineIntersection(l1)
        p3 = l4.lineIntersection(l1)
        p4 = l5.lineIntersection(l4)
        p5 = l5.lineIntersection(l6)

        allPoints = (p0, p1, p2, p3, p4, p5, point(p0.X, p0.Y))

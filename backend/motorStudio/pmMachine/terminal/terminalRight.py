...
class terminalRight(terminal):
    ...
    def __init__(self, stator, winding, phaseLetter='A', position=0, direction=terminalDirection.input):
        ...
        # self.getWireCoordinates()

    def getCoordinates(self, position=0):

        p = self.stator.sector.slot.getCoordinates()['mainPoints']
        l1 = line.__pointANDpoint__(p[len(p) - 2], p[len(p) - 3])
        l2 = line.__pointANDpoint__(p[len(p) - 3], p[len(p) - 4])
        l1.moveParallel(self.winding.slotIsolation)
        l2.moveParallel(self.winding.slotIsolation)
        l3 = line.__slopeANDpoint__(90, point(self.winding.phaseSeparation / 2, 0))
        l4 = line.__pointANDpoint__(p[len(p) - 4], p[len(p) - 5])
        l4.moveParallel(-self.winding.slotIsolation)

        segments = ()
        if (self.stator.type == statorType.stator1):
            c = circle(self.stator.outerDiameter / 2 - self.stator.sector.slot.yokeThickness - self.winding.slotIsolation)
            ...
            pt = p2.rotateArroundPointCopy(c.center, abs(
                p2.getRelativeSlope(c.center) - p3.getRelativeSlope(c.center)) / 2.0)
            ...
            l4 = line.__pointANDpoint__(p[len(p) - 4], p[len(p) - 5])
            l4.moveParallel(self.winding.slotIsolation)
            l5 = line.__slopeANDpoint__(0, p[len(p) - 2])
            ...
            l2.moveParallel(self.winding.slotIsolation)
            l3 = line.__slopeANDpoint__(90, point(self.winding.phaseSeparation / 2, 0))
            l4 = line.__pointANDpoint__(p[len(p) - 7], p[len(p) - 8])
            l4.moveParallel(-self.winding.slotIsolation)

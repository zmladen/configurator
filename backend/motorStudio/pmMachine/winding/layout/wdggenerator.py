...
def genwdg(Q, P, m, w, layers, empty_slots=0):
    ...
    if empty_slots == 0:
        ...
        if not ret["valid"] and empty_slots != 0:
            ret = winding_from_general_equation(
                Q, P, m, w, layers, empty_slots)
        if not ret["valid"]:
            ret = None
    elif empty_slots == -1:
            ...
            for km in range(m):
                ...
                for k in range(len(a[0])):
                    if not is_even(a[0][k]):  # use only the odd slots
                        a2[0].append(a[0][k])
                        # from second layer to first layer
                        a2[0].append(a[1][k])
                phases[km] = a2
        else:
...
def winding_from_general_equation(Q, P, m, w=-1, layers=2, n_es=0):
    ...
    if valid:
        ...
        q = fractions.Fraction((N - n_es) / (2 * p * m)
    ...
    if valid:
        v, x = check_number_of_coilsides(S)
        if not v:
            valid = False
        error += x

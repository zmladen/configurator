...
def get_basic_characteristics(Q, P, m, S, turns=1, Qes=0):
    ...
    if not valid:
        sym = False
    bc = {"q": q, "kw1": kw, "a": a, "sym": sym,
          "t": t, "lcmQP": lcmQP, "error": error}
    return bc
...
def calc_phaseangle_starvoltage(Ei):
    ...
    for a in Ei[0][0]:
        angle = np.angle(a) * 180 / np.pi
        print("angle = ", angle)
    phaseangle = []
    for knu in Ei:  # for every nu
        phaseangle.append([])
        # for every phase
        angle = [np.angle(sum(km)) * 180 / np.pi for km in knu]
        # print("phaseangle", angle)
        for a in angle:
            while a < 0:
                a += 360.0
...
def calc_kw_by_nu(Q, S, turns, p, nu):
    ...
    print("calc_phaseangle_starvoltage", phase, sequence, wf)
...
def calc_star(Q, S, turns, p, nu):
    ...
    for i, s in enumerate(S2):
        ...
        # print("phase ", i, "alpha_def", alpha * 180/math.pi)

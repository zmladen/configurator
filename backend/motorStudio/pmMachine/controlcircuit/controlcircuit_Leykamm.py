...
class controlcircuit(object):
    ...
    def __init__(self, data):
        ...
        self.tr = self.cc["Parts"]["Power Transistors"]["Parameters"][
            "Rise Time MOSFET (s)"
        ]
        self.tf = self.cc["Parts"]["Power Transistors"]["Parameters"][
            "Fall Time MOSFET (s)"
        ]
        self.ImaxTransistorRMS = self.cc["Parts"]["Power Transistors"]["Parameters"][
            "Maximal Current Rating RMS (A)"
        ]
        ...
        self.Cp = (
            self.cc["Parts"]["Intermediate Circuit Capacitors"]["Capacitance (F)"]
            * self.cc["Parts"]["Intermediate Circuit Capacitors"]["Total Number"]
        )
        self.Ls = (
            self.cc["Parts"]["Intermediate Circuit Inductances"]["Inductance (H)"]
            * self.cc["Parts"]["Intermediate Circuit Inductances"]["Total Number"]
        )

    def getLosses(self, Isource, Irms, Iripple):
        return {
            "Total Losses (W)": self.Pcontacts(Irms) + self.PpowerStage(Isource, Irms, Iripple),
    ...
    def Rshunt(self):
        ...
        Rshunt = (
            self.cc["Parts"]["Current Shunts"]["Resistance (Ohm)"]
    ...
    @property
    def Rind(self):
        Tc_Ind = self.cc["Parts"]["Intermediate Circuit Inductances"][
            "Resistance Tc (%/C)"
        ]
        Rind = (
            self.cc["Parts"]["Intermediate Circuit Inductances"]["Resistance (Ohm)"]
            * self.cc["Parts"]["Intermediate Circuit Inductances"]["Total Number"]
        )
    ...
    @property
    def Rrpp(self):
        Tc_rpp = self.cc["Parts"]["Reverse Polarity Protections"]["Parameters"][
            "Resistance Tc (%/C)"
        ]
        Rrpp = (

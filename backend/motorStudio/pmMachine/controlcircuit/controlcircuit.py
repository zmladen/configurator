...
class controlcircuit(object):
    ...
    def __init__(self, data):
        ...
        self.tr = self.cc["Parts"]["Power Transistors"][
            "Parameters"]["Rise Time MOSFET (s)"]
        self.tf = self.cc["Parts"]["Power Transistors"][
            "Parameters"]["Fall Time MOSFET (s)"]
        self.ImaxTransistorRMS = self.cc["Parts"]["Power Transistors"][
            "Parameters"]["Maximal Current Rating RMS (A)"]
        self.pwmFrequency = self.cc["Parts"][
            "Microcontroller"]["PWM Frequency (Hz)"]
        ...
        self.Cp = (self.cc["Parts"]["Intermediate Circuit Capacitors"]["Capacitance (F)"]
                   * self.cc["Parts"]["Intermediate Circuit Capacitors"]["Total Number"])
        self.Ls = (self.cc["Parts"]["Intermediate Circuit Inductances"]["Inductance (H)"]
                   * self.cc["Parts"]["Intermediate Circuit Inductances"]["Total Number"])
    ...
    def Rtransistor(self):
        Tc_Rtransistor = self.cc["Parts"]["Power Transistors"][
            "Parameters"]["Resistance Tc (%/C)"]
        Rtransistor = self.cc["Parts"]["Power Transistors"][
            "Parameters"]["Rdson (Ohm)"]
    ...
    def Rshunt(self):
        ...
        Rshunt = (self.cc["Parts"]["Current Shunts"]["Resistance (Ohm)"]
                  * self.cc["Parts"]["Current Shunts"]["Total Number"])
    ...
    @property
    def Rind(self):
        Tc_Ind = self.cc["Parts"][
            "Intermediate Circuit Inductances"]["Resistance Tc (%/C)"]
        Rind = (self.cc["Parts"]["Intermediate Circuit Inductances"]["Resistance (Ohm)"]
                * self.cc["Parts"]["Intermediate Circuit Inductances"]["Total Number"])
        return Rind * (1 + Tc_Ind / 100 * (self.temperature - 25))

    @property
    def Rrpp(self):
        Tc_rpp = self.cc["Parts"]["Reverse Polarity Protections"][
            "Parameters"]["Resistance Tc (%/C)"]
        Rrpp = (self.cc["Parts"]["Reverse Polarity Protections"]["Parameters"]["Rdson (Ohm)"]
                * self.cc["Parts"]["Reverse Polarity Protections"]["Total Number"])
        return Rrpp * (1 + Tc_rpp / 100 * (self.temperature - 25))

    @property
    def Rcond(self):
        Tc_Cond = self.cc["Parts"][
            "Intermediate Circuit Capacitors"]["Resistance Tc (%/C)"]
        Rcond = (self.cc["Parts"]["Intermediate Circuit Capacitors"]["Resistance (Ohm)"] /
                 self.cc["Parts"]["Intermediate Circuit Capacitors"]["Total Number"])

...
class foc(object):
    """Calculates the natural performance curve of the motor controlled by the so-called foc control"""

    def __init__(self, machine=None, speeds=[], currents=[], ids=[], iqs=[], temperatures=[25], numberofPeriods=1, numberofTimeSteps=120):
        ...
        if self.machine != None:
            ...
            self.__supplyVoltage = self.machine["design"]["Control Circuit"][
                "Used"]["Power Source"]["Supply Voltage (V)"]
        ...
        for temperature in self.temperatures:
            if createNewMaterials:
                ...
                for speed in self.speeds:
                    ...
                    path_speed = oProject.GetPath() + "/Results/foc/" + str(temperature) + \
                        "C" + "/" + str(speed) + "rpm"
                    ...
                    for id in self.ids:
                        self.id = id
                        path_id = path_speed + "/" + str(id) + "A_id"
                        if not os.path.exists(path_id):
                            os.makedirs(path_id)

                        for iq in self.iqs:
                            self.iq = iq
                            path = path_id + "/" + str(iq) + "A_iq"
                            ...
                            # path = oProject.GetPath() + "/Results/foc/" + str(temperature) + \
                            #     "C" + "/" + str(current) + "A"
                            # if not os.path.exists(path):
                            #     os.makedirs(path)
    ...
    def getOutputVariableDefinitions(self, partNames={}):
        ...
        outputVariables = [
            ...
            {'name': 'Simulation_Time', 'unit': 's', 'equation': 'Time',
                'type': 'Transient', 'integration factor': 0.5},
            {'name': 'Shaft_Position', 'unit': 'deg', 'equation': 'Moving1.Position/1deg %s' % (
                initialPosition), 'type': 'Transient', 'integration factor': 0.5},
            ...
            {'name': 'Shaft_Power', 'unit': 'W',
                'equation': 'deriv(Moving1.Position)*Moving1.Torque', 'type': 'Transient', 'integration factor': 0.5}
        ]
        ...
        # Add theequations for ud, uq, id, iq

        # I_d = 2/3 * (cos((Moving1.Position/1deg -10.0)*3.0*pi/180+pi) * InputCurrent(A) + cos((Moving1.Position/1deg -10.0)*3.0*pi/180+pi-2*pi/3) *  InputCurrent(B) + cos((Moving1.Position/1deg -10.0)*3.0*pi/180+pi+2*pi/3) * InputCurrent(C))

        name = 'ud'
        angle = "(Moving1.Position/1deg %s)*%s*pi/180+pi" % (initialPosition,
                                                             float(self.__poleNumber) / 2.0)
        equation = '2/3 * (cos(%s) * %s + cos(%s-2*pi/3) * %s + cos(%s+2*pi/3) * %s)' % (
            angle, "InducedVoltage(A)", angle, "InducedVoltage(B)", angle, "InducedVoltage(C)")
        ...
        for i in range(self.__phaseNumber):
            ...
            equation = 'InputCurrent(%s)' % (self.__phaseLetters[i])
            outputVariables.append(
        ...
        equation = ''
        for i in range(self.__phaseNumber):
            equation += '+%s*InputCurrent(%s)*InputCurrent(%s)' % (
        ...
        for i in range(self.__phaseNumber):
            ...
            equation = '%s*InputCurrent(%s)+InducedVoltage(%s)' % (
                self.__phaseResistance, self.__phaseLetters[i], self.__phaseLetters[i])
            outputVariables.append(
                {'name': name, "unit": "V", 'equation': equation, 'type': 'Transient', 'integration factor': 0.5})
        ...
        if self.__phaseConnection == "delta":
            for i in range(self.__phaseNumber):
                ...
                equation = 'InputCurrent(%s)-InputCurrent(%s)' % (
            ...
            for i in range(self.__phaseNumber):
                name = 'Line_Voltage_%s%s' % (
                    self.__phaseLetters[i], self.__phaseLetters[(i + 1) % self.__phaseNumber])
                equation = '%s*InputCurrent(%s)+InducedVoltage(%s)' % (
                    self.__phaseResistance, self.__phaseLetters[i].strip(), self.__phaseLetters[i].strip())
                outputVariables.append(
                    {'name': name, "unit": "V", 'equation': equation, 'type': 'Transient', 'integration factor': 0.5})

            # Shaft speed
            equation = 'deriv(Moving1.Position*1rad)*60/2/pi * ( %s / max(%s*InputCurrent(A)+InducedVoltage(A)))' % (
                self.__supplyVoltage, self.__phaseResistance)
            outputVariables.append({'name': 'Shaft_Speed', 'unit': 'rpm',
                                   'equation': equation, 'type': 'Transient', 'integration factor': 0.5})
            ...
            for i in range(self.__phaseNumber):
                name = 'Line_Voltage_%s%s' % (
                    self.__phaseLetters[i], self.__phaseLetters[(i + 1) % self.__phaseNumber])
                equation = '%s * InputCurrent(%s)+InducedVoltage(%s)-%s*Current(%s)-InducedVoltage(%s)' % (self.__phaseResistance, self.__phaseLetters[i], self.__phaseLetters[i],
            ...
            # Shaft speed
            equation = 'deriv(Moving1.Position*1rad)*60/2/pi * ( %s / max(%s * InputCurrent(A)+InducedVoltage(A)-%s*Current(B)-InducedVoltage(B)))' % (
                self.__supplyVoltage, self.__phaseResistance, self.__phaseResistance)
            outputVariables.append({'name': 'Shaft_Speed', 'unit': 'rpm',
                                   'equation': equation, 'type': 'Transient', 'integration factor': 0.5})
